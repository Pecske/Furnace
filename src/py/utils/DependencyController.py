...
class DependencyController:
    ...
    def build(self, instance_name: Type[T], params: Any | None = None) -> T:
        if instance_name in self.class_dict:
            ...
            if builder_has_param is not None:
                if (
                    params is not None
                    and len(builder_has_param) > 1
                    and builder_has_param[1]
                ):
    ...
    def __get_panel_service(self, db_name: str) -> PanelService:
        if PanelService not in self.dependencies:
            self.dependencies[PanelService] = PanelService(
                self.__get_panel_repo(db_name)
            )
    ...
    def __get_panel_name_service(self, db_name: str) -> PanelNameService:
        if PanelNameService not in self.dependencies:
            self.dependencies[PanelNameService] = PanelNameService(
                self.__get_panel_name_repo(db_name)
            )
    ...
    def __get_portion_service(self, db_name: str) -> PortionService:
        if PortionService not in self.dependencies:
            self.dependencies[PortionService] = PortionService(
                self.__get_portion_repo(db_name)
            )
    ...
    def __get_panel_controller(self, db_name: str) -> PanelController:
        if PanelController not in self.dependencies:
            self.dependencies[PanelController] = PanelController(
                self.__get_panel_service(db_name),
                self.__get_panel_name_service(db_name),
            )
    ...
    def __get_panel_name_controller(self, db_name: str) -> PanelNameController:
        if PanelNameController not in self.dependencies:
            self.dependencies[PanelNameController] = PanelNameController(
                self.__get_panel_name_service(db_name)
            )
    ...
    def __get_portion_controller(self, db_name: str) -> PortionController:
        if PortionController not in self.dependencies:
            self.dependencies[PortionController] = PortionController(
                self.__get_portion_service(db_name)
            )
    ...
    def __get_aggregator_controller(self, db_name: str) -> AggregatorController:
        if AggregatorController not in self.dependencies:
            self.dependencies[AggregatorController] = AggregatorController(
                self.__get_aggregator(db_name)
            )
