...
class Menu:
    ...
    DB_NAME = "src/data/Furnace.db"
    ADAG_PATH = "src/data/Adagok.csv"
    PANEL_PATH = "src/data/Hűtőpanelek.csv"
    ...
    def __map_panel_from_data(self, panel_names: list[PanelName], data : np.ndarray[tuple[Any,...],np.dtype[Any]]) -> list[Panel]:
    ...
    def __save_panel_batch(self, saved_names : list[PanelName], data : pd.DataFrame) -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            try:
                ...
                for split_data in split_datas:
                    futures.append(executor.submit(self.__map_panel_from_data,saved_names,split_data))
    ...
    def import_panels(self) -> None:
        ...
        if saved_names is not None:
            print(f"Panel name import finished without errors in: {name_end_time-name_start_time}!!!")
